...
class TransferMoneyView(LoginRequiredMixin):
    model = Transaction
    template_name = 'transactions/loan_request.html'
    ...
    def form_valid(self,form):
        ...
        if receiver_account==self.request.user.account:
            messages.error(
                self.request,'You cannot transfer money to your own'
        ...
        if not receiver_account:
            messages.error(
                self.request,'Receiver account is required'

            )
            return redirect('transfer_money')
        if not receiver_account.user:
            messages.error(
                self.request,'Receiver account not found'
